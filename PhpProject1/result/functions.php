...
function showNewsList(){
    ...
    $sql = "SELECT * FROM `guest_book`"; //конструируем запрос
    ...
    echo '<br/><a href="book.php">Раскрыть таблицу</a>';
}
...
function insertNews($title, $description=''){
    global $link;
    $sql = "INSERT INTO `guest_book` (`title`, `text`, `datetime`)
...
function showNewsForm($isUpdate, $title=null, $text=null){
    ...
    if($isUpdate){
        $sql = "SELECT * FROM `guest_book` WHERE `id` = {$_GET['id']}";

        $result = mysqli_query($link, $sql);
        $row = mysqli_fetch_row($result);

        echo '<form action="?action=update&id='.$_GET['id'].'" method="post">
    <label>Заголовок </label><input type="text" size="20" name="title" value="'.$row[1].'"/><br/><br/>
    <label>Текст новости</label><textarea rows="4" cols="20" name="text">'.$row[2].'</textarea><br/>
    <input type="submit" value="Обновить">
</form>';
    }
    else{
        echo '<form action="?action=insert" method="post">
    <label>Заголовок</label><input type="text" size="20" name="title" /><br/>
    <label>Текст новости</label><textarea rows="4" cols="20" name="text"></textarea><br/>
